        ...
        for N_TRAIN_CENS in "${N_TRAIN_CENS_LIST[@]}"; do
          for SURV_MODEL_TYPE in "${SURV_MODEL_TYPE_LIST[@]}"; do
            for CENS_MODEL_TYPE in "${CENS_MODEL_TYPE_LIST[@]}"; do
              ...
              JOBN="setup_${SETUP}/setting${SETTING}_surv_${SURV_MODEL_TYPE}_cens_${CENS_MODEL_TYPE}_train${N_TRAIN}_trainc${N_TRAIN_CENS}_cal${N_CAL}_batch${BATCH}.txt"
              ...
              if [[ $COMPLETE -eq 0 ]]; then
                ...
                SCRIPT="./experiment_1.sh $SETUP $SETTING $SURV_MODEL_TYPE $CENS_MODEL_TYPE $N_TRAIN $N_TRAIN_CENS $N_CAL $BATCH"
