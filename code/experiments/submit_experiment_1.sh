      ...
      for N_TRAIN in "${N_TRAIN_LIST[@]}"; do
        for SURV_MODEL_TYPE in "${SURV_MODEL_TYPE_LIST[@]}"; do
          for CENS_MODEL_TYPE in "${CENS_MODEL_TYPE_LIST[@]}"; do
            ...
            JOBN="setup_${SETUP}/setting${SETTING}_surv_${SURV_MODEL_TYPE}_cens_${CENS_MODEL_TYPE}_train${N_TRAIN}_cal${N_CAL}_batch${BATCH}.txt"
            ...
            if [[ $COMPLETE -eq 0 ]]; then
              ...
              SCRIPT="./experiment_1.sh $SETUP $SETTING $SURV_MODEL_TYPE $CENS_MODEL_TYPE $N_TRAIN $N_CAL $BATCH"
