...
df <- results %>% 
    ...
    group_by(dataset, surv_model_type, cens_model_type, alpha, Method, metric) %>%
    summarise(value.mean = mean(value), value.se = 2*sd(value)/sqrt(n())) %>%
    mutate(value = sprintf("%.3f (%.3f)", value.mean, value.se)) %>%
    select(-value.mean, -value.se) %>%
    pivot_wider(names_from = metric, values_from = value)


df %>% filter(dataset=="PBC")

df %>% filter(dataset=="VALCT")

df %>% filter(dataset=="GBSG")
