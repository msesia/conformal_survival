...
run_experiment <- function(random.state, random.state.data) {
    set.seed(random.state.data)
...
run_multiple_experiments <- function(batch_size) {
    ...
    random.state.data <- batch*1000
    # Loop over each repetition
    for (i in 1:batch_size) {
        random.state <- batch*1000 + i
        res <- run_experiment(random.state, random.state.data)  # Run experiment and get the result
